  ...
  return (
    <div className="container mx-auto py-4 sm:py-6 md:py-8 px-3 sm:px-4">
      <div className="flex flex-col items-center mb-6 sm:mb-8 md:mb-10">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-1 sm:mb-2 text-center">AI Learning Buddy</h1>
      ...
      <Tabs defaultValue="All" className="w-full mb-6 sm:mb-8" onValueChange={handleTabChange}>
        <TabsList className={`${isMobile ? (showMobileFilters ? 'flex' : 'hidden') : 'flex'} mb-4 sm:mb-6 flex-wrap`}>
          {domains.map(domain => (
            <TabsTrigger 
              key={domain} 
              value={domain} 
              className={`px-2 py-1 sm:px-4 sm:py-2 text-xs sm:text-sm ${isMobile ? 'flex-1' : ''}`}
            >
              {domain}
            </TabsTrigger>
          ))}
